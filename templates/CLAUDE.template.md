...
Every brand gets Google Search Console + Google Analytics 4 + Gmail wired under a single Google OAuth refresh token at setup time (Jon's SOP, locked 2026-05-10). One token, three APIs.

**Env vars used by the `gsc-ga4-sweep` skill (Monday 04:00 weekly):**
- `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REFRESH_TOKEN` — single OAuth covers all three APIs.
- `SITE_URL` — canonical brand URL with trailing slash (`https://yourdomain.com/`). Used to match the GSC URL-prefix property. Falls back to `WP_URL` with trailing slash if unset.
- `BRAND_NAME` — used for Airtable `Brand` field + brand-name query detection. Falls back to CLAUDE.md H1 if unset.
- `GA4_PROPERTY_ID` — optional. If unset, the skill enumerates `accountSummaries` and matches by domain. Set explicitly when multiple GA4 properties match the brand (e.g. UtamaSpice has 3 properties under one account).

**Scopes covered by the single token:**
- Gmail (`gmail.modify`, `gmail.compose`) — used by `email-checker` skill via `.claude/scripts/integrations/gmail.py`.
- Google Search Console (`webmasters.readonly`) — used by `gsc-ga4-sweep` for searchAnalytics queries.
- Google Analytics Data + Admin (`analytics.readonly`) — used by `gsc-ga4-sweep` for runReport + accountSummaries.

**Reference memory:** `reference_google_oauth_scope.md` documents the full curl pattern for each API.

**Multi-account caveat:** the brand's GSC property may live under a different Google account than `GOOGLE_REFRESH_TOKEN` covers — check by `GET /webmasters/v3/sites`. Empty `siteEntry` → wrong account. The `gsc-ga4-sweep` skill skips cleanly in that case; operator re-pairs the OAuth.
...
- Available env vars: `WP_URL`, `WP_USERNAME`, `WP_PASSWORD`, `AIRTABLE_API_KEY`, `AIRTABLE_BASE_ID`, `ASANA_API_KEY`, `REPLICATE_API`, `TINIFY_API`, `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET` (social media image CDN), `SENDY_API_KEY`, `SENDY_URL`, `METRICOOL_API_TOKEN`, `METRICOOL_BLOG_ID`, `METRICOOL_USER_ID`, `INSTANTLY_API_KEY`, `FIRECRAWL_API_KEY`, `JAMENDO_CLIENT_ID` (video pipeline BGM), `ELEVENLABS_API` (video pipeline voiceover), `ELEVENLABS_VOICE_ID`, `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `GOOGLE_REFRESH_TOKEN` (Gmail + GSC + GA4 single-token), `SITE_URL` (canonical brand URL for gsc-ga4-sweep), `BRAND_NAME` (for gsc-ga4-sweep Brand field), `GA4_PROPERTY_ID` (optional, gsc-ga4-sweep), plus all `AIRTABLE_*_TABLE` vars
